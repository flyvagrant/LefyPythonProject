...
def calculate_correlation(df1: pd.DataFrame, df2: pd.DataFrame, stock1_code: str, stock2_code: str) -> Tuple[float, float, float]:
    ...
    df_combined.to_csv(
        CSV_SAVE_PATH+stock1_code+"_"+stock2_code+"原始数据.csv",
        index=False,
        encoding="utf-8-sig"
    )
    # 保留4位小数
...
def main(stock_pairs: List[Tuple[str, str]]) -> None:
    ...
    for stock1_code, stock2_code in stock_pairs:
        try:
            ...
            # 3. 构造结果字典
            stock_pair_name = stock1_code+"-"+stock2_code  # f"{STOCK_MAPPING[stock1_code]}-{STOCK_MAPPING[stock2_code]}"
            result_data = {
                "股票对": stock_pair_name,
                "股票1代码": stock1_code,
                "股票2代码": stock2_code,
...
if __name__ == "__main__":
    ...
    #main([(stock1_code, stock2_code)])
    # 执行主逻辑
    stocks = xtdata.get_stock_list_in_sector('沪深A股')
    for stock_code in stocks:
        stock1_code="000300.SH"
        stock2_code=stock_code
        print(f"--- 处理股票对：{stock1_code} - {stock2_code} ---")
        main([(stock1_code, stock2_code)])